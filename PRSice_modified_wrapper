...
cat(" ################################# \n # \n # \n # \n # \n # Additional modification for HRS by Erin B. Ware - 2016 \n # \n # \n # \n #  \n ################################# \n")
...
ggfig <-  T
#ebw changed barchart levels
barchart.levels <- "0.001, 0.01, 0.05, 0.1, 0.3, 1.0"
barpalatte <- "YlOrRd"
best.thresh.on.bar <- F
scatter.R2 <- F						
figname <- "PRSice"
bar.col.is.pval <- T
bar.col.is.pval.lowcol <- "navyblue"
bar.col.is.pval.highcol <- "goldenrod1"
...
for(basePhen in 1:length(base.phenotypes.names)){
	...
	if(geno.is.ped & !geno.as.list){
	  cat(" ################################# \n # \n #   Reformat Target \n # \n ################################# \n")
	    system(paste(plink," --noweb --file", target," --keep ",keep.file," --make-bed --out", target))
	}
	if(geno.is.ped & geno.as.list){
	  cat(" ################################# \n # \n #   Reformat Target \n # \n ################################# \n")
	    for(i in 1:22){
	      file.name.chr <- paste(gsub(pattern="CHRNUM", replacement=i, x = target))
	      system(paste(plink," --noweb --file", file.name.chr," --keep ",keep.file," --make-bed --out", file.name.chr))
	    }
	}
	...
	cat(" ################################# \n # \n #   Remove Ambiguous SNPs \n # \n ################################# \n")
	  ...
	  if(!geno.as.list){ 
	    system(capture.output(cat("awk '{print $2,$5,$6}' ", target,".bim | sort -k1,1 | join -1 1 -2 1   temp.raw   ``-'' | awk '($4 == \"A\" && $5 == \"T\" ||$4 == \"T\" && $5 == \"A\" || $4 == \"C\" && $5 == \"G\"  || $4 == \"G\" && $5 == \"C\"  ){print $1}' > ambiguous_snps  \n ", sep = ""))[1])
	  system(paste( plink, "        --noweb --bfile ", target," --exclude ambiguous_snps  --keep ",keep.file," --make-bed   --out non_ambiguous_snps_only", sep = ""))
	  }
	  if(geno.as.list){
	  	for(chrnum in 1:22){
	  	  file.name.chr <- paste(gsub(pattern="CHRNUM", replacement=chrnum, x = target))
	      system( capture.output(cat("awk '{print $2,$5,$6}' ", file.name.chr,".bim | sort -k1,1 | join -1 1 -2 1   temp.raw   ``-'' | awk '($4 == \"A\" && $5 == \"T\" ||$4 == \"T\" && $5 == \"A\" || $4 == \"C\" && $5 == \"G\"  || $4 == \"G\" && $5 == \"C\"  ){print $1}' > ambiguous_snps_",chrnum,"  \n ", sep = ""))[1])
	      system(paste( plink, "        --noweb --bfile ", file.name.chr," --exclude ambiguous_snps_",chrnum,"  --keep ",keep.file," --make-bed   --out non_ambiguous_snps_only_",chrnum, sep = ""))
	    }
	    system("cat non_ambiguous_snps_only_*.bim  > non_ambiguous_snps_only.bim")  
	  }
	}
	...
	if(!dosage){
	  system(paste("awk '{print $2}' non_ambiguous_snps_only.bim | sort -k 1,1 >  ./TARGET_SNPs", sep = ""))
	...
	if(clump.snps & !dosage & !(geno.as.list)){
			## clump and extract independent snps
			system(paste(plink," --noweb --keep ",keep.file," --bfile non_ambiguous_snps_only --clump cleaned_base.assoc --clump-p1 ",clump.p1," --clump-p2 ",clump.p2," --clump-r2 ",clump.r2," --clump-kb ",clump.kb," --out cleaned_base", sep=" "))
	  ...
	  if(length(dir()[grep("cleaned_base.assoc", dir())]) == 0 ){
	    cat("ERROR: Clumping Failed, check discovery data set format \n "); quit()
	  }
	...
	if(clump.snps & !dosage & geno.as.list){
			## clump and extract independent snps
			for(chrnum in 1:22){
			  system(paste(plink," --noweb --keep ",keep.file," --bfile non_ambiguous_snps_only_",chrnum," --clump cleaned_base.assoc --clump-p1 ",clump.p1," --clump-p2 ",clump.p2," --clump-r2 ",clump.r2," --clump-kb ",clump.kb," --out cleaned_base_",chrnum, sep=""))
			}
...
#	system(paste(plink," --allow-no-sex --noweb --dosage ", gen.name ,"  noheader	skip0=1	skip1=1	skip2=0 	format=3   --fam ", target.fam, "  --score rawfile.raw   --q-score-range rangelist.txt rangelist_ranges") )
	  ...
	  ## clump and extract independent snps
	  system(paste(plink," --noweb --keep ",keep.file," --dosage ", gen.name, " noheader format=",dos.format," dose",dos.coding," skip0=",dos.skip0," skip1=",dos.skip1," --fam ", target.fam,"  --clump cleaned_base.assoc --clump-p1 ",clump.p1," --clump-p2 ",clump.p2," --clump-r2 ",clump.r2," --clump-kb ",clump.kb," --out cleaned_base", sep=""))
	  system("tail -n +2 cleaned_base.clumped | awk '{print $3}'  | sort -k1,1 | awk '($1 != \"\"){print}'  > LE_SNPs")
	  ## linkage equilibrium snps
	  if(!use.beta){
	  system(paste("tail -n +2 cleaned_base.assoc | sort -k1,1 | join -1 1 -2 1 LE_SNPs ``-'' | awk '{print $",which(head.disc=="SNP"),
	...
	if(prune.snps){
	  ...
	  if(!dosage & !(geno.as.list)){
	    ## prune and extract independent snps
	    system(paste(plink," --noweb --keep ",keep.file," --bfile non_ambiguous_snps_only --indep-pairwise ",prune.kb.wind," kb ",prune.kb.step, prune.kb.r2," --out cleaned_base", sep=" "))
	  ...
	  if(!dosage & (geno.as.list)){
	    ## prune and extract independent snps
	    for(chrnum in 1:22){
	      system(paste(plink," --noweb --keep ",keep.file," --bfile non_ambiguous_snps_only_",chrnum," --indep-pairwise ",prune.kb.wind," kb ",prune.kb.step," ", prune.kb.r2," --out cleaned_base_",chrnum, sep=""))
	    }
	...
	if(prune.snps & dosage & dosage.format == "gen"){
	  ## prune and extract independent snps
	  system(paste(plink," --noweb --keep ",keep.file," --dosage ", gen.name, " noheader format=",dos.format," dose",dos.coding," skip0=",dos.skip0," skip1=",dos.skip1," --fam ", target.fam,"  --indep-pairwise ",prune.kb.wind," ",prune.kb.step," ",prune.kb.r2," --out cleaned_base", sep=""))
	  system("tail -n +2 cleaned_base.clumped | awk '{print $3}'  | sort -k1,1 | awk '($1 != \"\"){print}'  > LE_SNPs")
	  ## linkage equilibrium snps
	  if(!use.beta){
	  system(paste("tail -n +2 cleaned_base.assoc | sort -k1,1 | join -1 1 -2 1 LE_SNPs ``-'' | awk '{print $",which(head.disc=="SNP"),
	...
	if(!dosage){
	  ## strand flips
	  system(paste("awk '{print $2,$5,$6}' non_ambiguous_snps_only.bim | sort -k1,1 | join -1 1 -2 1   rawfile.raw   ``-'' | awk '($2 != $4 && $2 != $5) {print $1}' > flip_list.txt", sep = 	""))
    ## Complete records
	  system(paste("awk '{print $2}' non_ambiguous_snps_only.bim | sort -k1,1 | join -1 1 -2 1   rangelist_ranges   ``-''  > Complete_Allele_List.txt", sep = 	""))
	  ComAlLi <- read.table("Complete_Allele_List.txt", head=F)
    names(ComAlLi) <- c("SNP", "P")
	  	  	  
	  if(!(geno.as.list)){ 
	    system(paste( plink, "        --noweb --bfile non_ambiguous_snps_only --flip flip_list.txt ",mhc,"  --make-bed   --keep ",keep.file,"  --out flipped_target", sep = ""))
	  }
	  if(geno.as.list){
	  	for(chrnum in 1:5){ 
	      system(paste( plink, "        --noweb --bfile non_ambiguous_snps_only_",chrnum," --flip flip_list.txt  --make-bed  --keep ",keep.file," --out flipped_target_",chrnum, sep = ""))
	    }
	  	for(chrnum in 6){ 
	      system(paste( plink, "        --noweb --bfile non_ambiguous_snps_only_",chrnum," --flip flip_list.txt ",mhc,"  --make-bed  --keep ",keep.file," --out flipped_target_",chrnum, sep = ""))
	    }
	  	for(chrnum in 7:22){ 
	      system(paste( plink, "        --noweb --bfile non_ambiguous_snps_only_",chrnum," --flip flip_list.txt   --make-bed --keep ",keep.file,"  --out flipped_target_",chrnum, sep = ""))
	    }
	...
	cat(" ################################# \n # \n #   Polygenic scoring! \n # \n ################################# \n")
				...
				#ebw - added SUM indicator to get the PGS SUM
	if(!dosage & !(geno.as.list)){
		system(paste(plink, "        --noweb --bfile flipped_target --keep ",keep.file,"  --score rawfile.raw  sum --q-score-range rangelist.txt rangelist_ranges  --out PROFILES"))
	}
	
	if(!dosage & (geno.as.list)){
	  for(chrnum in 1:22){
	    system(paste(plink, "        --noweb --bfile flipped_target_",chrnum,"   --keep ",keep.file," --score rawfile.raw sum --q-score-range rangelist.txt rangelist_ranges  --out ",chrnum,"PROFILES",sep=""))
	  }
	}
     if(dosage & is.na(dos.path.to.lists) & is.na(dos.list.file) & dos.coding == 1){
 system(paste(plink," --noweb --keep ",keep.file," --dosage ", gen.name, " noheader format=",dos.format," dose",dos.coding," skip0=",dos.skip0," skip1=",dos.skip1," --fam ", target.fam,"  --score rawfile.raw sum --q-score-file rangelist_ranges --q-score-range rangelist.txt   --out PROFILES", sep = ""))
 }
     if(dosage & is.na(dos.path.to.lists) & is.na(dos.list.file) & dos.coding == 2){
           system(paste(plink," --noweb --keep ",keep.file," --dosage ", gen.name, " noheader format=",dos.format," skip0=",dos.skip0," skip1=",dos.skip1," --fam ", target.fam,"  --score rawfile.raw sum --q-score-file rangelist_ranges --q-score-range rangelist.txt   --out PROFILES", sep = ""))
     }
	...
	if(dosage & (!is.na(dos.path.to.lists) | !is.na(dos.list.file))){
	  for(i in 1:22){
	    ...
	    system(paste(plink," --noweb --keep ",keep.file," --dosage ",gen.name," noheader format=",dos.format," dose",dos.coding," skip0=",dos.skip0," skip1=",dos.skip1," --fam ", target.fam,"  --score rawfile.raw sum --q-score-file rangelist_ranges --q-score-range rangelist.txt   --out ",i,"PROFILES", sep = ""))  
	  }
	}            
	  ...
	  if(binary.target){
	    for(i in 1:length(lists$V1)){
	      ...
	      if((!dosage & !geno.as.list)  | (dosage & is.na(dos.path.to.lists) & is.na(dos.list.file))){			
	        ...
	        prof <- prof[,c("IID", "PHENO", "SCORESUM")]
			 names(prof) <- c("IID", "PHENO", "SCORE")
	      }
	      if(ext.phen){
		  ...
		  prof.all.scores<-prof.all.scores[!duplicated(prof.all.scores$IID),]
	...
	if(no.regression & !multiple.target.phenotypes){
	  ...
	  if(cleanup){
	    ...
	    system("rm *PROFILES.S8*.profile", ignore.stdout=T,ignore.stderr=T)
		#Added
	    system("rm *PROFILES.nopred", ignore.stdout=T,ignore.stderr=T)
	    ...
	    system("rm PROFILES.log", ignore.stdout=T,ignore.stderr=T)
	    system("rm ambiguous_snps*", ignore.stdout=T,ignore.stderr=T)
	    system("rm ambiguous_snps", ignore.stdout=T,ignore.stderr=T) 
	    system("rm non_ambiguous_snps_only*", ignore.stdout=T,ignore.stderr=T)
